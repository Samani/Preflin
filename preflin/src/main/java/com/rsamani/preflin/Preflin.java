...
import com.rsamani.preflin.serializer.DefaultSerializer;
...
public class Preflin {
    ...
    private static HashMap<String, PreferenceFunction> prefHashMap;
    ...
    public static void init(Context ctx, int defaultCommitBehavior, Serializer serializer) {
        ...
        prefHashMap.put(DEFAULT, new PreferenceFunction(PreferenceManager.getDefaultSharedPreferences(mContext), serializer));
    ...
    public static Observable<String> listenOn(String key) {
        PreferenceFunction pref = prefHashMap.get(DEFAULT);
        return pref.listenOn(key);
    }

    public static PreferenceFunction from(String name) {
        ensureInit();
        return from(name, Context.MODE_PRIVATE);
    }

    public static PreferenceFunction from(String name, int mode) {
        ensureInit();
        PreferenceFunction prefInternal;
        if (prefHashMap.containsKey(name)) {
            prefInternal = prefHashMap.get(name);
        } else {
            prefInternal = new PreferenceFunction(mContext.getSharedPreferences(name, mode), mSerializer);
